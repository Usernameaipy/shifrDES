...
uint64_t **creat_new_matrix(int num_str) {
  int p_trig = 0;
  uint64_t **mem_point =
      malloc(num_str * sizeof(uint64_t) + num_str * sizeof(uint64_t *));
...
void delete_matrix(uint64_t **matrix) { free(matrix); }

uint64_t join_message_in_matrix(uint8_t *message_8b) {
  uint64_t block;
  for (uint8_t *p = message_8b;
       (p < message_8b + 8 && *p != '\0') || (p < message_8b + 8 && *p == '\0');
       ++p) {
    block = (block << 8) | *p;
  }
  return block;
